...
# Get number of samples of 'top' category (label 0) and their indices
label = 0
...
generator.add(UpSampling2D(size=(2, 2)))
generator.add(Conv2D(input_shape[0], kernel_size=(5, 5), padding='same', activation='tanh'))
...
discriminator.add(Conv2D(64, kernel_size=(5, 5), strides=(2, 2), padding='same', input_shape=input_shape, kernel_initializer=initializers.RandomNormal(stddev=0.02), name='CONV1', trainable=False))
discriminator.add(LeakyReLU(0.2))
discriminator.add(Dropout(0.3))

discriminator.add(Conv2D(128, kernel_size=(5, 5), strides=(2, 2), padding='same', name='CONV2', trainable=False))
discriminator.add(LeakyReLU(0.2))
discriminator.add(Dropout(0.3))

discriminator.add(Conv2D(256, kernel_size=(3, 3), strides=(2, 2), padding='same', name='CONV3', trainable=False))
discriminator.add(LeakyReLU(0.2))
discriminator.add(Dropout(0.3))
...
# Create a wall of generated FMNIST images
...
def train(epochs=1, batchSize=128):
    ...
    for e in range(1, epochs+1):
        ...
        for _ in tqdm(range(int(batchCount))):
            ...
            imageBatch = imageBatch.reshape(imageBatch.shape[0], input_shape[0], input_shape[1], input_shape[2])
        ...
        print ('Epoch :', e, ' D Loss: ', dloss, ' G Loss: ', gloss)
        ...
        if e == 1 or e % 5 == 0:
            ...
            ############# Extra for now #######################
            noise = np.random.normal(0, 1, size=[3000, randomDim])
            generatedImages = generator.predict(noise)
            np.save(('images/x_train_class_%d.npy' % label), generatedImages)
