...
# Get number of samples of 'dog' category (label 5) and their indices
label = 5
...
# Delete a random 60% of the class
delete_index = sample(range(len(X_train)), int(0.6 * class_size))
...
generator.add(Dense(512*8*8, input_dim=randomDim, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
generator.add(BatchNormalization(momentum=0.9))
generator.add(LeakyReLU(0.2))
generator.add(Reshape((512, 8, 8)))
...
#generator.load_weights('models/dcgan_generator_epoch_6.h5')
...
#discriminator.load_weights('models/dcgan_discriminator_epoch_6.h5')
...
def train(epochs=1, batchSize=128):
    ...
    for e in range(1, epochs+1):
        ...
        for _ in tqdm(range(int(batchCount))):
            ...
            # Generate fake MNIST images and create mixed batch to train discriminator on
            generatedImages = generator.predict(noise)
        ...
        print ('Epoch %f D Loss: %f G Loss: %f' % (e, dloss, gloss))
